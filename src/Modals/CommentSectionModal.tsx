...
const CommentSectionModal = ({ open, setShowModal, posts }: propstype) => {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => getAllCommentsById(posts.id),
  });

  const toggleModal: ToggleModal = () => setShowModal(!open);
  const addComments = async () => {
    await addCommentsApi({ post_id: posts.id, comment: addComment });
      ...
      >
        ...
        <div className="basis-2/4  flex flex-col justify-between">
          <div className=" flex p-3">
            <img
              src={posts.owner.profile_photo}
              alt="p"
              className="w-[25px] h-[25px] rounded-full"
              style={{ border: "1px solid black" }}
            />
            <b className="ml-3">{posts.owner.username}</b>
          </div>
